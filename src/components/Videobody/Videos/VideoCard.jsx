...
export const VideoCard = () => {
    ...
    const formatViewsCount = (viewsCount) => {
        if (viewsCount < 1000) {
            ...
            const decimal = remainder >= 100000 ? `.${Math.floor(remainder / 100000)}` : '';
            return `${millions}${decimal}m`;
        } else {
            const billions = Math.floor(viewsCount / 1000000000);
            const remainder = viewsCount % 1000000000;
            const decimal = remainder >= 100000000 ? `.${Math.floor(remainder / 100000000)}` : '';
    ...
    return (
        <div className="flex flex-row flex-wrap pt-24 px-3">
            {searchResults.map((data) => (
                <section className="flex flex-col justify-center m-2 rounded-lg" key={data.id}>
                ...
                <div className="flex flex-col justify-start">
                    ...
                    <div className="flex space-x-2">
                        <p className="text-gray-400 text-sm">{formatViewsCount(data.statistics.viewCount)} Views</p>
